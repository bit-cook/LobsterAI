...
- 未输入内容且正在录音时，输入框 placeholder 隐藏，居中展示“支持中英混合，正在识别中...” tips、波形动画和计时。
...
- 未输入内容的录音态由输入区中心承载 tips、波形和计时；已有输入内容时由底部操作区承载波形和计时。
...
- 统一渲染波形动画和计时，保证未输入/已输入两种录音态视觉一致。
...
- 未输入内容录音时隐藏 placeholder 和输入框内部工具入口，居中展示 tips、波形和计时。
